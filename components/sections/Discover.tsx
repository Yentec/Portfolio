...
import { cvRows, rpgStats, rpgCharacter, discoverContent } from "@/content/discover";
  ...
  return (
    <article className="flex h-full flex-col gap-4 rounded-lg border border-[rgba(1,22,39,0.08)] bg-white p-8 text-[#011627] shadow-[0_30px_60px_-34px_rgba(0,0,0,0.55)]">
      ...
      <div className="flex flex-1 flex-col">
        {cvRows.map((row, i) => (
          ...
          >
            ...
            <span className="text-right text-[14px] font-semibold text-[#011627]">{row.value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="mt-1.5 grid grid-cols-1 gap-2.75 sm:grid-cols-2">
        ...
        <a href="#contact">
          <Button className="w-full justify-center">{discoverContent.cv.ctaContact}</Button>
        </a>
      </div>
    </article>
  );
...
function RpgCard() {
  return (
    <article
      ...
      style={{
        background: "linear-gradient(180deg, rgba(234,241,247,.07), rgba(234,241,247,.015))",
      }}
    >
      ...
      <div className="relative flex flex-1 flex-col gap-3.25 rounded-[15px] border border-[rgba(234,241,247,0.12)] bg-[rgba(3,16,31,0.55)] px-5 py-4.5">
        ...
        <p className="text-accent font-mono text-[12.5px]">{rpgCharacter.characterClass}</p>
      ...
      <div className="max-w-site relative z-1 mx-auto w-full px-7">
        ...
        <Reveal>
          <div className="mb-11.5">
            <Eyebrow className="text-accent">{discoverContent.eyebrow}</Eyebrow>
            <h2 className="text-[clamp(28px,3.8vw,44px)] text-white">{discoverContent.title}</h2>
            <p className="mt-4 text-justify text-[17px] text-[rgba(234,241,247,0.74)]">
              {discoverContent.lead}
            </p>
          </div>
        </Reveal>
